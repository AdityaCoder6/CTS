...
const sendOTP = async (email) => {
  const otp = Math.floor(100000 + Math.random() * 900000); // Generate 6 digits OTP
  ...
  // Define mail options
  const mailOptions = {
    from: 'authcts@gmail.com', // Sender's email address
    to: email, // Receiver's email address
    subject: 'OTP for Registration in Cross The Skylimits', // Email subject
    text: `Hello, Your OTP for registration is ${otp}` // Email body
  };
...
const registerUser = asyncHandler(async (req, res) => {
  ...
  // Create a new user with OTP and profilePicDetails if applicable
